...
VQA_EMERGENCY_VEHICLE = "is there an ambulance or a fire truck?"
...
def process_video_streaming(
    ...
    try:
        ...
        for i, res in enumerate(frame_results):
            ...
            is_ambulance_warning = (
                (bool(session_ambulance_tids) or vqa.get("blip_ambulance") == "yes")
                and not is_anom
            ...
            # save screenshot (use annotated_pil if available)
            screenshot = None
            if is_anom or is_ped_crosswalk or is_ambulance_warning:  # added is_ambulance_warning
                if is_vehicle_crosswalk:
                    color = "orange"
                elif is_ped_crosswalk:
                    color = "yellow"
                elif is_ambulance_warning:  # yellow for ambulance warning
                    color = "yellow"
                else:
                    color = "red"
            ...
            frame_payload = {
                ...
                "is_ambulance_warning": is_ambulance_warning,   # NEW
            }
            if screenshot:
            ...
            if is_ambulance_warning:
                sio.emit("ambulance_warning", {
            ...
            # emit traffic-specific SocketIO events
            if traffic_info and traffic_info.get("score", 0) >= 0.25:
                ...
                # Dedicated ambulance alert channel (only for critical/high)
                if (session_ambulance_tids
                        or "ambulance" in t_type_emit.lower()
                        or vqa.get("blip_ambulance") == "yes"):
                    # Distinguish between critical ambulance events and normal presence
                    if t_sev_emit in ("CRITICAL", "HIGH"):
                        # Critical situation -> ambulance_alert (red)
                    ...
                    # else: already handled by ambulance_warning above
